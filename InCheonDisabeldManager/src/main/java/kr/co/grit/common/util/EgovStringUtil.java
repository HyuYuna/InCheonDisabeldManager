...
public class EgovStringUtil {
	...
	/**
	 * <p>접수번호 형식의 문자열 내부에 마이너스 character(-)를 추가한다.</p>
	 *
	 * <pre>
	 *   StringUtil.addMinusCharRcptNo("000000000000000000") = "0000-0000000-0000000"
	 * </pre>
	 *
	 * @param data  입력받는 문자열
	 * @return " - "가 추가된 입력문자열
	 */
	public static String addCharRcptNo(String data) {
		if (data.length() == 18) {
			return data.substring(0, 4).concat("-").concat(data.substring(4, 11)).concat("-").concat(data.substring(11, 18));
		} else {
			return data;
		}
	}
	...
	/**
	 * <p>납부번호 형식의 문자열 내부에 마이너스 character(-)를 추가한다.</p>
	 *
	 * <pre>
	 *   StringUtil.addMinusCharPaymNo("0000000000000000") = "0000000-0000-000000"
	 * </pre>
	 *
	 * @param data  입력받는 문자열
	 * @return " - "가 추가된 입력문자열
	 */
	public static String addMinusCharPaymNo(String data) {
		if (data.length() == 16) {
			return data.substring(0, 7).concat("-").concat(data.substring(7, 10)).concat("-").concat(data.substring(10, 16));
		} else {
			return data;
		}
	}
	...
	/**
	 * 아임스타즈 시스템 연계 개발(2017.11.10)
	 * 
	 * 주어진 자리수 만큼 대문자 A~Z, 소문자 a~z, 숫자 0~9 사이의 랜덤 문자열을 구하는 기능을 제공
	 * 옵션 : 날짜값 포함 여부(중복을 막기 위해)
	 *
	 * @param bDateInculde - 날짜값 포함 여부(중복을 막기 위해)
	 * @param iLength - 랜덤 문자열 자리수
	 * @return 랜덤문자
	 */
	public static String getRandomStrNum(boolean bDateInculde, int iLength) {

		String randomStr = null;
		String sTimestamp = null;
		int checkIlength = iLength;
		...
		// 타임스탬프 날짜값(17자리) 포함시 문자열 앞부분에 추가 
		if(bDateInculde == true && checkIlength > 17) {
			sTimestamp = getTimeStamp();
			checkIlength = checkIlength - 17;
			sbTemp.append(sTimestamp);
		}

		// 랜덤 문자 숫자 생성
		for (int i = 0; i < checkIlength; i++) {
		    int rIndex = rnd.nextInt(3);
		    switch (rIndex) {
		    case 0:
		        // a-z
		    	sbTemp.append((char) ((int) (rnd.nextInt(26)) + 97));
		        break;
		    case 1:
		        // A-Z
		    	sbTemp.append((char) ((int) (rnd.nextInt(26)) + 65));
		        break;
		    case 2:
		        // 0-9
		    	sbTemp.append((rnd.nextInt(10)));
		        break;
		    }
		}
		
		// 랜덤 문자 숫자 스트링으로 변환
		randomStr = sbTemp.toString();
