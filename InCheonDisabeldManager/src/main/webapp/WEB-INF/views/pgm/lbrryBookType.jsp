...
%>
                ...
                <table class="dataTable marginT10">
                    ...
                    <tbody>
                        <tr>
                            <th>도서유형코드(명)</th>
                            <td>
								<input type="text" id="schTypeCd" name="schTypeCd" style="width:100%;" title="도서유형명" maxlength="5" 
                ...
                <table class="layoutTable">
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="subtitleGroup">
                                <div class="subtitle">유형 정보</div>
                                <div class="subBtnGroup">
                                </div>
                            </div>
                            <!-- <h2>유형 정보</h2> -->
                            
							<form name="bookTypeForm" id="bookTypeForm" method="post" autocomplete="off">
								...
								<input type="hidden" id="rowIndex"   name="rowIndex"  value=""/>
									...
									<tr>
										<th>유형코드</th>
										<td>
											<input type="text" id="typeCd" name="typeCd" style="width:40%; text-align: left;"  title="유형코드" readonly> 
										</td>
									</tr>
									<tr>
										<th><span class="essentialItem">유형명</th>
										<td>
											<input type="text" id="typeNm" name="typeNm" style="width:40%; text-align: left;"  title="유형명" onkeyup="checkByte(this, 50)" onkeyup="checkByte(this, 80)" onkeydown="<%=layerName%>EnterFilter('useFg');" /> 
										</td>
									</tr>
									<tr>
...
<script type="text/javascript">
	...
	$('.section-wrap .boxcontent.<%=layerName%> .titleWrap .btnMainGroup .save').click(function(){
		<%=layerName%>fn_saveLbrryBookType();
	});
	...
	var <%=layerName%>columnLayout = [ 
		 {dataField : "typeCd", headerText : "유형코드", width : "30%"}
	    ,{dataField : "typeNm", headerText : "유형명", width : "70%", style:"my-column-left"}
	    ];
	...
	function <%=layerName%>fn_newAll() {
		if ( confirm ("신규로 입력 하시겠습니까?") ) {
			$(".<%=layerName%> #bookTypeForm")[0].reset();
	...
	//저장전 검사
	function <%=layerName%>fn_saveLbrryBookType(){
		if ( $(".<%=layerName%> #typeNm").val() == "" ) {
			$(".<%=layerName%> #typeNm").focus();
			GRIT.msg.alert("[유형명]은 필수 항목입니다."); return;
		}
	...
	function <%=layerName%>fn_save(){
		...
		var sendData = $(".<%=layerName%> form[name=bookTypeForm]").serializeObject();

         GRIT.tran.send({
			url: "/pgm/lbrryBookTypeSave",
			async: false,
			sendData: sendData,
