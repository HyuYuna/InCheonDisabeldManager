...
%>
                ...
                <form id="searchWffcltyForm" name="searchWffcltyForm" onsubmit="return false;" autocomplete='off'>
                <table class="dataTable marginT10">
                    ...
                    <tbody>
                        <tr>
                            <th>복지시설</th>
                            <td colspan="3">
                            	<input type="text" id="searchWffcltyCd" name="searchWffcltyCd" title="복지시설"  oninput="<%=layerName%>fn_removeWffcltyNm();" onkeydown="<%=layerName%>fn_searchWffcltyCd(this.value)">
                ...
                <!-- conditionTable : start -->
                <form id="baseWffcltyForm" name="baseWffcltyForm" onsubmit="return false;" autocomplete='off'>
                <table class="dataTable marginT10">
                    <colgroup>
                        <col style="width:140px">
                        <col style="width:140px">
                    ...
                    <tbody>
                        ...
                        <h2>시설 기본 정보</h2>
                            <th>시설코드</th>
                             <td>
                            	<input type="text" id="baseWffcltyCd" name="baseWffcltyCd"  title="시설코드" disabled>
                            </td>
                            <th>시설명</th>
                            <td>
                            	<input type="text" id="baseWffcltyNm" name="baseWffcltyNm"  style="width: 100%" title="시설명" disabled>
                            </td>
                           
                            <th>시설유형</th>
                            <td>
                            	<input type="text" id="baseWffcltyTpNm" name="baseWffcltyTpNm"   title="시설유형" disabled>
                            </td>
                            <th>운영단체</th>
                            <td>
                            	<input type="text" id="baseOgnztNm" name="baseOgnztNm"  title="운영단체" disabled>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <th>고유번호</th>
                            <td>
                            	<input type="text" id="baseUniqueNo" name="baseUniqueNo"  title="고유번호" disabled>
                            </td>
                            <th>본부/센터</th>
                            <td>
                            	<input type="text" id="baseHighWffcltyNm" name="baseHighWffcltyNm" style="width: 100%"  title="본부/센터" disabled>
                            </td>	 
                             
                            <th>운영여부</th>
                            <td>
                            	<input type="text" id="baseOperateFg" name="baseOperateFg"  title="운영여부" tabindex="1" disabled>
                            </td>
                            <th>사용여부</th>
                            <td>
                            	<input type="text" id="baseUseFg" name="baseUseFg"  title="사용여부" tabindex="1" disabled>
                            </td>
                        </tr>
                    </tbody>
                </table>
                    ...
                    <tr>
                           ...
                           <div class="subtitleGroup">
                             ...
                             <form name="detailForm" id="detailForm" method="post" autocomplete="off">
								<table cellpadding="0" cellspacing="0" class="dataTable" id="align_common" border="1">
										...
										<input type="hidden" id="hiddenWffcltyTp"  name="wffcltyTp"        value="" />
                        ...
                        <td>
                            ...
                            <table class="dataTable">
                            	<div id="<%=layerName%>grid_wrap" style="height: 240px;"></div>
                            </table>
                            ...
                            <table class="dataTable">
                            	<div id="<%=layerName%>grid_wrap2" style="height: 239px;"></div>
                            </table> 
                        </td>
...
<script type="text/javascript">
		...
		GRIT.tran.send({
			...
			success: function(result){
				...
				if(result.wffcltyDetailList.length > 0){
					...
					var wffcltyTp = $(".<%=layerName%> #hiddenWffcltyTp").val();
	...
	function <%=layerName%>setSearchWffcltyPop(item,  name){
		...
		// 시설기본정보
		$(".<%=layerName%> #baseWffcltyCd").val(item.wffcltyCd);
		$(".<%=layerName%> #baseWffcltyNm").val(item.wffcltyNm);
		$(".<%=layerName%> #baseWffcltyTpNm").val(item.wffcltyTpNm);
		$(".<%=layerName%> #baseOgnztNm").val(item.ognztNm);
		$(".<%=layerName%> #baseUniqueNo").val(item.uniqueNo);
		$(".<%=layerName%> #baseHighWffcltyNm").val(item.highWffcltyNm);
		$(".<%=layerName%> #baseOperateFg").val(item.operateFg);
		$(".<%=layerName%> #baseUseFg").val(item.useFg);
		
		// 복지시설코드,시설유형
		$(".<%=layerName%> #wffcltyCd").val(item.wffcltyCd);
		$(".<%=layerName%> #hiddenWffcltyTp").val(item.wffcltyTp);
	...
	//복지시설
	function <%=layerName%>fn_searchWffcltyCd(value){
	   ...
	   if(event.keyCode != 13 && event.keyCode != 9){
        ...
        GRIT.tran.send({
			url: "/wfc/selectWffcltyList",
			async: false,
			sendData: sendData,
			success: function(result){
				if(result.data.length > 1){
					if($(".<%=layerName%> #searchWffcltyNm").val() != '' || $(".<%=layerName%> #searchWffcltyNm").val().length != 4){
					...
					// 시설기본정보
					$(".<%=layerName%> #baseWffcltyCd").val(result.data[0].wffcltyCd);
					$(".<%=layerName%> #baseWffcltyNm").val(result.data[0].wffcltyNm);
					$(".<%=layerName%> #baseWffcltyTpNm").val(result.data[0].wffcltyTpNm);
					$(".<%=layerName%> #baseOgnztNm").val(result.data[0].ognztNm);
					$(".<%=layerName%> #baseUniqueNo").val(result.data[0].uniqueNo);
					$(".<%=layerName%> #baseHighWffcltyNm").val(result.data[0].highWffcltyNm);
					$(".<%=layerName%> #baseOperateFg").val(result.data[0].operateFg);
					$(".<%=layerName%> #baseUseFg").val(result.data[0].useFg);
					
					// 복지시설코드, 시설유형
					$(".<%=layerName%> #wffcltyCd").val(result.data[0].wffcltyCd);
					$(".<%=layerName%> #hiddenWffcltyTp").val(result.data[0].wffcltyTp);
	...
	// reset
	function <%=layerName%>fn_removeWffcltyNm(){
