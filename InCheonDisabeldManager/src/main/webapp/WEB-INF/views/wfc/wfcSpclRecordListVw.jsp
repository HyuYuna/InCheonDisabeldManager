...
%>
				...
				<input type="hidden" id="<%=layerName%>groupType" name="groupType" value="${groupType}" />
					<table class="dataTable marginT10">
	                    ...
	                    <c:if test="${userGroupCd == 'A' || userGroupCd == 'B' || userGroupCd == 'E'}">
	                    	...
	                    	<tr>
								<th>장애인</th> 
								<td class="title">
				                            	<input type="text" id="searchDspsnId" name="searchDspsnId" style="width:40%;" maxlength="10" title="장애인" oninput="<%=layerName%>fn_removeDspsnNm()" onkeydown="<%=layerName%>fn_searchAjaxDspsn(this.value)" >
					            				<button type="button" class="iconSearchGlassBtn" onclick="<%=layerName%>fn_selectDspsn(); return false;" style="margin-top: 2px">
					            					<img src="/resources/imagesV2/iconSearchGlass.png" class="iconSearchGlass">
					            				</button>
					        					<input type="text" id="searchDspsnNm" name="searchDspsnNm" style="width:50%;" title="장애인명"  disabled>
								</td>
								
								<th>기록일</th>
								<td class="title">
									<input type="text" id="<%=layerName%>sDate" name="fDate" style="width: 100px;" class="datepicker hover_disabled" maxlength="10" onkeypress="<%=layerName%>fn_pressSys(this.form); " onblur="<%=layerName%>checksDate(this.value)" onKeyup="inputYMDNumber(this);" onKey> 
									<span style="margin-left:30px;">~</span>
									<input type="text" id="<%=layerName%>eDate" name="tDate" style="width: 100px; margin-left:5px;" class="datepicker hover_disabled" maxlength="10" onkeypress="<%=layerName%>fn_pressSys(this.form);" onblur="<%=layerName%>checkeDate(this.value)" onKeyup="inputYMDNumber(this);">
								</td>
								
								<th>사례구분</th>
								<td class="title">
									<select id="searchUnusalFg" name="searchUnusalFg" style="width: 100%; text-align-last : center;">
					...
					<tr> 
					<h2>특이사항 리스트</h2>
						<div id="<%=layerName%>grid_wrap" style="height: 235px;"></div>
					</tr>
							    ...
							    <h2>특이사항 기록</h2>
				                    ...
				                    <table cellpadding="0" cellspacing="0"  class="dataTable" id="align_common" border="1" >
				                        ...
				                        <tr>
				                            ...
				                            <td style = "border-right: none;">
				                            	<input type="text" id="spcDspsnId" name="spcDspsnId" style="width:40%;" title="장애인" tabindex="4" disabled>
					        					<input type="text" id="spcDspsnNm" name="spcDspsnNm" style="width:59%; margin-left: 0px" title="장애인명"  disabled> 
				                            </td>
				                            <td colspan="2" style = "border-left: none;"/>
				                            <td rowspan="4">
				                            	<div class="<%=layerName%>image-container">
				                            		<img id="img-ListVw" src=" ">
				                            	 </div>
				                            </td>
				                            </tr>
				                            <tr>
					                            <th>생년월일</th>
					                            <td><input type="text" id="encBirthDd" name="encBirthDd" style="width:100%;text-align:left;" title="생년월일" tabindex="1" disabled> 
				                            </td>
					                            <th>성별</th>
					                            <td><input type="text" id="spcGender" name="spcGender" style="width:100%;text-align:left;" title="성별" tabindex="1" disabled> </td>
				                        </tr>
				                        <tr>
					                        <th>장애유형</th>
					                         <td><input type="text" id="spcDisableTp" name="disableTp" style="width:100%;text-align:left;" title="장애유형" tabindex="1" disabled> </td>
					                        <th>청각문제</th>
					                         <td><input type="text" id="spcHealthSts03" name="spcHealthSts03" style="width:100%;text-align:left;" title="청각문제" tabindex="1" disabled> </td>
				                        </tr>
				                        <tr>
					                        <th>장애정도</th>
				                            <td><input type="text" id="spcDisableDgreeFg" name="spcDisableDgreeFg" style="width:100%;text-align:left;" title="장애정도" tabindex="1" disabled> </td>
				                        	<th>시각문제</th>
				                            <td><input type="text" id="spcHealthSts01" name="spcHealthSts01" style="width:100%;text-align:left;" title="시각문제" tabindex="1" disabled> </td>
				                        </tr>
				                        <tr>
					                        <th>발생시기</th>
					                            <td><input type="text" id="spcOutbreakTimeFg" name="spcOutbreakTimeFg" style="width:100%;text-align:left;" title="발생시기" tabindex="1" disabled> </td>
					                         <th>언어문제</th>
					                            <td style ="border-right: none">
					                            	<input type="text" id="spcHealthSts02" name="spcHealthSts02" style="width:100%;text-align:left;" title="언어문제" tabindex="1" disabled>
					                            </td>
										 ...
										 <td>
				                            <select id="unusalFg" name="unusalFg" style="width: 100%; text-align-last : center;" onchange="<%=layerName%>selectBoxList();" disabled>
													<c:if test="${!empty csmnDvsnList}">	
														<c:forEach var="list" items="${csmnDvsnList}">
															<option value="${list.csmnDvsnCd}">
																<c:out value="${list.csmnDvsnNm}" /></option>
										...
										<th>질환</th>
					                            <td style ="border-right: none">
					                            	<input type="text" id="spcHealthSts06" name="spcHealthSts06" style="width:100%;text-align:left;" title="질환" tabindex="1" onkeydown="<%=layerName%>EnterFilter('actTp');" disabled>
					                            </td>
				                        		<td style ="border-left: none"/>
				                        </tr>
				                        <tr>
				                    		...
				                    		<td colspan = "3" style ="border-right: none">
					                            <select id="actTp" name="actTp" style="width: 100%; text-align-last : center;" tabindex="1" onchange="<%=layerName%>actTpDetail();" disabled>
												...
												<input type="text" id="actTpDetail" name="actTpDetail" style="width:100%;text-align:left;" title="기타행동유형" tabindex="1" disabled>
												</td>
												 <td style ="border-left: none"/>
				                    	</tr>
				                    	 <tr>
				                    		<th>조치내역</th>
					                            <td colspan = "3" style ="border-right: none">
					                            	<input type="text" id="measureDetail" name="measureDetail" style="width:100%;text-align:left;" title="조치내역" tabindex="1" disabled>
					                            </td>
				                    			<td style ="border-left: none"/>
				                    	</tr>
...
<script type="text/javascript" >
	...
	// 전역변수 값 설정
	var <%=layerName%>actTp ="01";
	
	var <%=layerName%>columnGroupType = $(".<%=layerName%> #<%=layerName%>groupType").val();
	
	var <%=layerName%>columnLayout  = [
		{dataField : "wffcltyNm" ,headerText : "복지시설"       ,width : "11%", style : "my-column-left" , headerStyle:"auiGridHeader"}
       ...
       ,{dataField : "unusalFgNm"      ,headerText : "사례구분"    ,width : "12%", style : "my-column-left", headerStyle:"auiGridHeader"}
       ,{dataField : "actTpNm"      ,headerText : "행동유형"    ,width : "18%", style : 	"my-column-left", headerStyle:"auiGridHeader"} 
       ,{dataField : "measureDetail"      ,headerText : "조치사항"    ,width : "28%", style : "my-column-left", headerStyle:"auiGridHeader"}
       ];
	...
	$(function () {
		...
		$("#unusalFg option:selected").val("U1");
	...
	function <%=layerName%>setFieldToFormWindow(primeCell, item) {
		...
		$(".<%=layerName%> #spcDspsnId").val(item.dspsnId);
		$(".<%=layerName%> #hdnDspsnId").val(item.dspsnId);
		$(".<%=layerName%> #spcDspsnNm").val(item.dspsnNm);
		...
		$(".<%=layerName%> #img-ListVw").val(item.picturePath);
	...
	function <%=layerName%>loadData () {
		if( $(".<%=layerName%> #wffcltyCd").val() != ""){
			...
			GRIT.msg.alert("조회 조건중 [복지시설]을 조회 후 해당 복지시설을 조회해주세요."); return;
			}
		}
		...
		GRIT.tran.send({
			url: "/wfc/selectWfcSpclRecordListVwGrid",
			sendData : sendData,
	...
	function <%=layerName%>fn_newSys() {
		...
		$(".<%=layerName%> #spcDspsnId").val("");
		$(".<%=layerName%> #spcDspsnNm").val("");
		...
		$(".<%=layerName%> #unusalFg").val("U1");
		$(".<%=layerName%> #spcHealthSts06").val("");
		$(".<%=layerName%> #actTp").val("01");
	...
	function <%=layerName%>fn_saveEnextMgmt() {
		...
		if ( $(".<%=layerName%> #spcDspsnId").val() == "" || $(".<%=layerName%> #spcDspsnNm").val() == "" ) {
			$(".<%=layerName%> #spcDspsnId").focus();
			GRIT.msg.alert("[이용 장애인]은 필수 항목입니다."); return;
		}
		
		if ( $(".<%=layerName%> #<%=layerName%>unusalFg").val() == "" ) {
			$(".<%=layerName%> #<%=layerName%>unusalFg").focus();
			GRIT.msg.alert("[사례구분]는 필수 항목입니다."); return;
		}
		
		if ( $(".<%=layerName%> #<%=layerName%>actTp").val() == "" ) {
			$(".<%=layerName%> #<%=layerName%>actTp").focus();
 ...
 // 장애인 팝업2
	...
	function <%=layerName%>exportTo() {
		...
		if(excelList.length == 0){
			GRIT.msg.alert("특이사항 리스트 조회된 내용이 없습니다.");
			return false;
		}
